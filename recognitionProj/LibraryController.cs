...
namespace RecognitionProj.Controllers
{
    ...
    public class LibraryController : ControllerBase
    {
        ...
        // GET: api/library/getbyid/{id}
        [HttpGet("getbyid/{insId}")]
        public IActionResult GetLibraryById(int insID)
        {
            var result = _dbHandler.GetLibraryById(insID);
            if (result == null)
            {
                return NotFound(new { success = false, message = $"No Library found for InsID {insID}." });
            }
