document.addEventListener('DOMContentLoaded', () => {
     ...
     *         DeletePath: "/api/nonconventional/delete?filePath=/uploads/{institutionId}/attachments/FileName.pdf"
     *       }
     */
    async function loadFiles() {
        try {
            const response = await fetch('/api/nonconventional/list', {
            ...
            data.files.forEach((file, index) => {
                /*
                  Each file object has:
                    ...
                    file.DeletePath => "/api/nonconventional/delete?filePath=/uploads/9999/attachments/01_CH1 - Control Systems Basics, tst.pdf"
                */
    ...
    fileUploadForm.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch('/api/nonconventional/upload', {
                method: 'POST',
                body: formData,
                headers: {
     ...
     *    DELETE /api/nonconventional/delete?filePath=...
     */
    filesTableBody.addEventListener('click', async (e) => {
        if (e.target.classList.contains('delete-btn')) {
            const deletePath = e.target.dataset.file; // e.g. "/api/nonconventional/delete?filePath=/uploads/9999/attachments/SomeFile.pdf"
